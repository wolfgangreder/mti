 ...
 * @author wolfi
 */
public class DerbyLocomotiveItemProvider extends AbstractMotrivDerbyItemProvider<UUID, Locomotive> implements LocomotiveItemProvider
{

  private static class MyInitializer
  {

    private static DerbyLocomotiveItemProvider instance = new DerbyLocomotiveItemProvider();
  }

  public static DerbyLocomotiveItemProvider getInstance()
  {
    return MyInitializer.instance;
  }

  private DerbyLocomotiveItemProvider()
  {
  }
  ...
  Map<UUID, ? extends Locomotive> get(Connection conn, Collection<UUID> ids) throws SQLException, DataProviderException
  {
    ...
    if (!ids.isEmpty()) {
      ...
      try {
        ...
        DerbyContactItemProvider contactProvider = DerbyContactItemProvider.getInstance();
        DerbyScaleItemProvider scaleProvider = DerbyScaleItemProvider.getInstance();
        for (UUID id : ids) {
