...
public final class Fract extends Number implements Comparable<Fract>
{
  ...
  public static final Fract ZERO = new Fract(0,
                                             1,
                                             false);
  public static final Fract ONE = new Fract(1,
                                            1,
                                            false);
  public static final Fract HO = new Fract(1,
                                           87,
                                           false);

  public static Fract valueOf(long numerator,
                              long demnominator) throws IllegalStateException
  {
  ...
  public static Fract valueOf(long numerator,
                              long demnominator,
                              boolean cancelDown) throws IllegalStateException
  {
    if (demnominator == 0) {
  ...
  public Fract cancelDown()
  {
    long gcd = gcd(numerator,
                   denominator);
    if (gcd != 1) {
      return new Fract(numerator / gcd,
                       denominator / gcd,
                       false);
    } else {
      return this;
  ...
  public Fract multiply(Fract f,
                        boolean cancelDown)
  {
    return valueOf(numerator * f.numerator,
  ...
  public Fract multiply(long l,
                        boolean cancelDown)
  {
    return valueOf(numerator * l,
                   denominator,
                   cancelDown);
  }

  public static double multiply(Fract f,
                                double d)
  {
    if (f.isZero() || d == 0d) {
      return 0d;
  ...
  public static BigDecimal multiply(Fract f,
                                    BigDecimal d)
  {
    ...
    tmp = tmp.multiply(d);
    return tmp.divide(BigDecimal.valueOf(f.denominator),
  ...
  public Fract divide(Fract f,
                      boolean cancelDown) throws IllegalArgumentException
  {
    if (f.isZero()) {
      throw new IllegalArgumentException("division by zero");
    }
    return valueOf(numerator * f.denominator,
  ...
  {
    return MessageFormat.format("Fract:{0,number,#}/{1,number,#}",
                                numerator,
                                denominator);
  }
