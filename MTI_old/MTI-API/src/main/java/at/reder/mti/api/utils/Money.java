...
public final class Money extends Number implements Comparable<Money>
{
  ...
  private static final MathContext MATH_CONTEXT = new MathContext(80, RoundingMode.HALF_UP);
  ...
  {
    return valueOf(value.divide(curr.value, MATH_CONTEXT));
  }

  public Money divide(BigDecimal curr)
  {
    return valueOf(value.divide(curr, MATH_CONTEXT));
  }

  public Money modulo(Money curr)
  {
    return valueOf(value.remainder(curr.value, MATH_CONTEXT));
  }

  public Money modulo(BigDecimal curr)
  {
    return valueOf(value.remainder(curr, MATH_CONTEXT));
  }
  ...
  private BigDecimal innerRoundTo(int nDigits, boolean divide)
  {
    BigDecimal dig;
    switch (nDigits) {
      case 0:
    ...
    tmp = tmp.setScale(0, ROUNDING_MODE);
    if (divide) {
  ...
  {
    return Money.valueOf(innerRoundTo(nDigits, true));
  }

  public BigInteger toCent()
  {
    return new BigInteger(innerRoundTo(2, false).toPlainString());
  }
